...
export const TrialExpiredScreen = ({ onSubscribe, onSignIn }) => {
  const C = useTheme();

  // ── Native iOS variant ─────────────────────────────────────────────────────
  // App Store guideline 3.1.1: no in-app subscribe CTAs, no price mentions,
  // no links to the web. iOS users who want to subscribe must do so on the
  // web independently, then sign in here.
  if (isNativeIOS()) {
    return (
      <div style={{ minHeight:"100vh", background:C.bg, fontFamily:FONT, display:"flex", flexDirection:"column", justifyContent:"center", padding:"0 20px" }}>
        <div style={{ maxWidth:400, margin:"0 auto", width:"100%", textAlign:"center" }}>
          ...
          <p style={{ color:C.muted, fontSize:15, lineHeight:1.7, marginBottom:24 }}>
            Sign in to your LeanPlan account to continue.
          </p>
          <Btn onClick={onSignIn} color={C.accent} style={{ width:"100%", fontSize:17, padding:"16px 0", marginBottom:12 }}>
            Sign In
          </Btn>
        </div>
      </div>
