...
app.post("/api/send-shopping-list", async (req, res) => {
  ...
  const listHtml = categories.map(cat => `
    <tr>
      <td colspan="2" style="padding:14px 0 4px; font-size:11px; font-weight:700; letter-spacing:0.08em; color:#636366; text-transform:uppercase; border-top:1px solid #2c2c2e;">${cat.name}</td>
    </tr>
    ${cat.items.map(item => `
      <tr>
        <td style="padding:8px 0; font-size:14px; color:#ffffff; border-bottom:1px solid #2c2c2e;">• ${item.display}</td>
        <td style="padding:8px 0; font-size:13px; color:#636366; text-align:right; border-bottom:1px solid #2c2c2e;">${item.amounts?.[0] || ""}</td>
      </tr>
    `).join("")}
  `).join("");
  ...
  const html = `
    ...
    <div style="max-width:600px;margin:0 auto;">
      ...
      <!-- Footer -->
      <div style="background:#111113;padding:20px 32px;border-radius:0 0 16px 16px;text-align:center;">
        <p style="color:rgba(255,255,255,0.35);font-size:12px;margin:0;">Generated by LeanPlan · <a href="https://www.leanplan.uk" style="color:#0a84ff;text-decoration:none;">leanplan.uk</a></p>
      </div>
